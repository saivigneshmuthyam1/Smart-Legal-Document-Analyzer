...
def generate_report(document_id: str, summary, risks, clauses, metadata, options=None) -> str:
    if options is None:
        options = {"inc_summary": True, "inc_risks": True, "inc_clauses": True, "inc_meta": True}
    """
    ...
    # Cover / Header
    story.append(Paragraph("LEXICON AI | DOCUMENT INTELLIGENCE", title_style))
    story.append(Paragraph("Automated Legal Review & Risk Analysis Audit Report", subtitle_style))
    story.append(Spacer(1, 10))
    
    # Metadata summary table
    if options.get("inc_meta", True):
    ...
    # 1. Executive Summary
    if options.get("inc_summary", True):
    ...
    # 3. Clause Analysis
    if options.get("inc_clauses", True):
